...
const resolvers = {
  Query: {
    ...
    allBooks: (root, { author, genre }) => {
      const genreBooks = genre ? books.filter(book => book.genres.includes(genre)) : books
      const authorBooks = author ? genreBooks.filter(book => book.author === author) : genreBooks
      return authorBooks
    },
